...
def näita():
    ...
    nimetused =  scrapimine(kuupäev1, kuupäev2, 'a.eventslist_item_title', '43172')
    kuupäev = scrapimine(kuupäev1, kuupäev2, 'div.eventslist_item_date', '43172')
    piletihind = scrapimine(kuupäev1, kuupäev2, 'div.eventslist_item_price_primary', '43172')
    ...
    gif1 = PhotoImage(file = 'LinksPildid/Pärnu/PärnuTeater.gif')
    canvas.create_image(150,0, image = gif1, anchor = NW)

    lst = Listbox(app, activestyle='dotbox',\
              fg='black', highlightbackground='blue',\
              highlightcolor='green', width=70, heigh=10)
    lst.insert(END,"Endla teater:")
    lst.insert(END, "        ")
    ...
    def Link():
        webbrowser.open("http://www.endla.ee/")
    button = ttk.Button(app, text = "Endla teater", command = Link)
    button.pack()
